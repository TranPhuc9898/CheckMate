...
import { onGetMyTasks, onUpdateMyTasksSocket } from "screens/tab-home-my-tasks/slice";
...
const TaskWaitingReturn = ({ navigation }) => {
  ...
  const [tasksOfDate, setTasksOfDate] = useState(
    _.filter(listDataTask, (item) => (moment(item.date).isSame(moment(), "day") && item?.isReceived))
  );
  ...
  return (
    <>
      <FlatList
        ...
        ListHeaderComponent={<DateOfWeek onChange={setDateFilter} />}
...
export default TaskWaitingReturn;
