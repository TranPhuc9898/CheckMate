...
const DateOfWeek = ({ onChange, isWaitingCollect }) => {
  ...
  const shouldRenderWeekday = useMemo(() => {
    return dates.map((date, idx) => {
      ...
      const numberTaskOfDate = _.filter(listDataTask, (item) => {
        const isSameDate = moment(item.date).isSame(date, "day");
        return isWaitingCollect ? !item?.isReceived && isSameDate : isSameDate;
      }).length;
