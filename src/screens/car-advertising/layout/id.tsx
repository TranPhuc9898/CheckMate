...
const CarAdvertisingScreen = () => {
  ...
  const _onShareReferralCode = async () => {
    ...
    try {
      ...
      const referralValue = I18n.t("CAR_ADVERTISING.COST_AND_CURRENCY", {
        cost: formatMoney(inviteeValue),
        currency: country?.currency.sign,
      });
      const txtVoucher = I18n.t("CAR_ADVERTISING.COST_AND_CURRENCY", {
        cost: formatMoney(voucherValue),
        currency: country?.currency.sign,
      });
      // const message = I18n.t('CAR_ADVERTISING.INVITE_ASKER_TEXT_HEAD') + ' ' + referralCode + ' ' + I18n.t('CAR_ADVERTISING.INVITE_ASKER_TEXT_TAIL', { t: cost });
      // referralSetting
