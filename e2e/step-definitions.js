...
const urlServer = "http://localhost:3344/api/";
...
const loginWithPhoneAndPassword = async (phone, password, isoCode) => {
  try {
    await tapText("Đóng");
  } catch (error) {
  }
  ...
  if(isoCode){
    await element(by.id("ChooseCountry")).tap();
    await element(by.id(`country${isoCode}`)).tap();
    await element(by.id("btnNextToLogin")).tap();
    await tapId("chooseLanguage");
    await tapId("language_vi");
  }
  await element(by.id("TextInputPhone")).tap();
  await element(by.id("TextInputPhone")).clearText();
  await element(by.id("TextInputPhone")).typeText(phone);
  await element(by.id("TextInputPassword")).clearText();
  await element(by.id("TextInputPassword")).typeText(password);
...
const fillActiveCode = async (phone, countryCode = "+84") => {
  const data = await initData("user/getActivationCode", { phone: phone, countryCode: countryCode });
  const code = data.code
  await element(by.id("activationCode")).typeText(code);
...
const scroll = async (id, pixels, direction) => {
...
const loginWithThaiLanPhoneNumber = async (phone, password) => {
  await tapId("btnDropdownMenuCode");
  if (device.getPlatform() === "ios") {
    await tapText("Thái Lan(+66)");
  } else {
    await tapText("+66");
  }
...
const tapWithoutFeedback = async (value, type) => {
  try {
    if (type === "text") {
      await tapText(value);
    } else {
