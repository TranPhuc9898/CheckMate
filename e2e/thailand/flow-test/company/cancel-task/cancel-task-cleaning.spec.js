...
describe("FILE: flow-test/cancel-task/tasker-cancel-task-cleaning-th.spec.js - Tasker cancel task cleaning th", () => {
  beforeEach(async () => {
    ...
    await initData("update-user/financialAccount", {
      phone: "0834567891",
      isoCode: "TH",
      financialAccountData: { TH_FMainAccount: 10000, TH_Promotion: 10000 },
    });
  ...
  it("LINE 35 - Tasker cancel task have reson NEARBY_TASK_PLACE", async () => {
    await initData("th/task/createTask", {
      resetCollection: true,
      serviceName: "CLEANING",
      viewedTaskers: ["0834567891"],
  ...
  it("LINE 70 - Tasker cancel task have fee", async () => {
    ...
    await tapId("btnCancelTask");
    await expectElementVisible("cancelSuccess");
    // const notify = await initData("/notification/get-notification-indo", { phone: '0834567891', description: 'My Task' });
    // expect(notify[0].description).equal('Lý do hủy: Xem nhầm ngày. Tài khoản của bạn bị trừ 20,000 VND theo qui định của bTaskee.');
  });
  ...
  it("LINE 108 - Tasker cancel task with other reason", async () => {
    ...
    await expectElementVisible("cancelSuccess");
    // const notify = await initData("/notification/get-notification-indo", { phone: '0834567891', description: 'My Task' });
    // expect(notify[0].description).equal('Lý do hủy: Lý do bất khả kháng. Tài khoản của bạn bị trừ 20,000 VND theo qui định của bTaskee.');
  });
  ...
  it("LINE 165 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await tapId("reasonCancelWRONG_DATE");
    await expectElementVisible("cancelFee");
    await expectElementVisible("200฿", "text");
    await tapId("btnCancelTask");
    await expectElementVisible("cancelSuccess");
  });
  ...
  it("LINE 165 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await tapId("Finance");
    await expectIdToHaveText('transactionHistory-0', '+50฿');
    await tapId("TransactionHistory");
    await expectElementVisible("Lịch sử giao dịch", "text");
    await expectIdToHaveText('detailTransaction-0', '+50฿');
  });
  ...
  it("LINE 345 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await expectElementVisible("Lịch sử giao dịch", "text");
    // Bị trừ tiền
    await expectElementVisible("Bạn không tới làm mà không có báo trước", "text");
    await expectIdToHaveText('detailTransaction-0', '-1,000฿');
  });
  ...
  it("LINE 382 - Tasker cancel task and status task change to CANCELED", async () => {
    const ref = await initData("th/task/createTask", {
      ...
      date: moment().subtract(0, "d").hour(0).minute(0).toDate(),
      duration: 2
    });
  ...
  it("LINE 382 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await expectElementVisible("Tài khoản chính", "text");
    // Được cộng thêm 800
    await expectElementVisible("10,800฿", "text");
    // Trừ 200 của 20% task
    await expectElementVisible("-200฿", "text");
  });
});
