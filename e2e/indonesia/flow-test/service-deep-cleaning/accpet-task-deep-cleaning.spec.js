 ...
 * @desc [Tasker accept tassk cleaing id]
...
describe("FILE: indonesia/flow-test/service-deep-cleaning/accept-task-deep-cleaning.spec.js - Tasker accept task deep cleaning id", () => {
  beforeEach(async () => {
  ...
  it("LINE 81 - Tasker see and accept task deep cleaning id, see in waiting task", async () => {
  ...
  it("LINE 130 - Tasker see and accept task deep cleaning id, see in confirmed task -> tasker is leader", async () => {
    const refUser = await initData("/user/createUser", {
      phone: "0834567999",
      isoCode: "ID",
      resetUser: true,
  ...
  it("LINE 190 - Tasker see and accept task deep cleaning id has newCostDetail, see in confirmed task -> tasker is leader", async () => {
    const refUser = await initData("/user/createUser", {
      phone: "0834567999",
      isoCode: "ID",
      resetUser: true,
  ...
  it("LINE 279 - Accept task deep cleaning limit is 2 with status confirm", async () => {
    await initData("id/services/update", {
      serviceName: "DEEP_CLEANING",
      dataUpdate: {
    ...
    await swipe("swipeBtn", "right");
    await expectElementVisible(
      "Để đảm bảo sức khỏe cho CTV và chất lượng dịch vụ cho Khách hàng, bTaskee giới hạn mỗi CTV chỉ được nhận tối đa 2 công việc Tổng Vệ Sinh/ngày.",
      "text"
    );
  });
});
