 ...
 * @desc [Tasker cancel task cleaning indonesia]
...
describe("FILE: indonesia/flow-test/service-cleaning/cancel-task-cleaning.spec.js - Tasker cancel task cleaning indonesia", () => {
  beforeEach(async () => {
    await device.launchApp();
    await initData("update-user/financialAccount", {
      phone: "0834567891",
      isoCode: "ID",
      financialAccountData: { ID_FMainAccount: 100000, ID_Promotion: 1000 },
    });
    await initData("update-user/unset-fields", { phone: "0834567891", isoCode: "ID", fields: ["taskCancelByTasker"] });
  });
  ...
  it("LINE 70 - Tasker cancel task have fee", async () => {
    ...
    await expectIdToHaveText("cancelFee", "20,000Rp")
  ...
  it("LINE 198 - Tasker cancel task cleaning and status task change to CANCELED", async () => {
    ...
    await tapId("btnConfirmCancel");
    await tapId("reasonCancelWRONG_DATE");
    await expectIdToHaveText("cancelFee", "50,000Rp")
    await tapId("btnCancelTask");
    await expectElementVisible("cancelSuccess");
    await tapText("Đóng");
    await tapId("TabAccount");
    // Kiểm tra lại tài khoản
    await tapId("Finance");
    await expectIdToHaveText('transactionHistory-0', '-50,000Rp');
  });
