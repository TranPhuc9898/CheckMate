...
describe("FILE: indonesia/flow-test/service-cleaning/accept-task.spec.js - Tasker accept task cleaning indonesia", () => {
  beforeEach(async () => {
  ...
  // Thấy công việc và nhận công việc
  it("LINE 175 - Tasker accept task CLEANING indonesia", async () => {
    await initData("id/task/createTask", {
      resetCollection: true,
      serviceName: "CLEANING",
      viewedTaskers: ["0834567891"],
  ...
  it("LINE 239 - Tasker accept task cleaning not enough money", async () => {
    ...
    await initData("update-user/financialAccount", {
      phone: "0834567891",
      isoCode: "ID",
      financialAccountData: { ID_FMainAccount: 100, ID_Promotion: 100 },
    });
    await device.reloadReactNative();
    await waitForElement("newTask_GoMarket", 1000);
    await tapId("newTask_GoMarket");
  ...
  it("LINE 318 - Tasker accept task have newCostDetail", async () => {
    ...
    await initData("update-user/financialAccount", {
      phone: "0834567891",
      isoCode: "ID",
      financialAccountData: { ID_FMainAccount: 500000, ID_Promotion: 50000 },
    });
    await initData("id/updateTask", {
      taskId: ref.taskId,
      dataUpdate: {
    ...
    await expectIdToHaveText("extraMoney", "+199,000Rp Thu tiền mặt từ khách hàng");
    await swipe("scrollTaskDetail", "up");
    await expectElementVisible("btn_chat");
    await expectElementVisible("btn_call");
    await expectElementVisible("btn_cancel");
  ...
  it("LINE 378 - Tasker accept task cleaning with promotion", async () => {
    ...
    await waitForElement("newTask_My Task01", 1000);
    await tapId("newTask_My Task01");
    await expectIdToHaveText("cost_0", "200,000Rp");
    await expectElementNotExist("promotionValue");
    await swipe("scrollTaskDetail", "up");
    await swipe("swipeBtn", "right");
    await tapHeaderBack();
    await tapId("confirmTask_My Task01");
    await expectIdToHaveText("cost_0", "180,000Rp");
    await expectIdToHaveText("promotionValue", "+20,000Rp Thêm vào TK khuyến mãi của bạn");
  });
});
