 ...
 * case 1: Tasker see and accept task office cleaning th, see in waiting task
 * case 2: Tasker see and accept task office cleaning th, see in confirmed task -> tasker is member
 * case 3: Tasker see and accept task office cleaning th, see in confirmed task -> tasker is leader
 * case 4: Tasker can not accept task if number of TaskDone < 20
 * case 5: Tasker can not accept task if AvgRating = 4.6
 * case 6: Accept task office cleaning limit is 2 with status confirm
 */
...
describe("FILE: vietnam/flow-test/service-office-cleaning/accept-task-office-cleaning.spec.js - Tasker accept task office cleaning vn", () => {
  beforeEach(async () => {
  ...
  it("LINE 81 - Tasker see and accept task office cleaning vn, see in waiting task", async () => {
  ...
  // Lỗi
  it("LINE 67 - Tasker see and accept task office cleaning th, see in confirmed task -> tasker is member", async () => {
  ...
  it("LINE 130 - Tasker see and accept task office cleaning vn, see in confirmed task -> tasker is leader", async () => {
    const refUser = await initData("/user/createUser", {
      phone: "0834567999",
      isoCode: "VN",
      resetUser: true,
  ...
  it("LINE 190 - Tasker see and accept task office cleaning vn has newCostDetail, see in confirmed task -> tasker is leader", async () => {
    const refUser = await initData("/user/createUser", {
      phone: "0834567999",
      isoCode: "VN",
      resetUser: true,
    ...
    await tapHeaderBack();
    await tapId("confirmTask_My Task");
    await scroll("scrollTaskDetail", 500, "down");
    await expectElementNotExist("callTasker0");
    await expectElementVisible("callTasker1");
  ...
  it("LINE 279 - Accept task office cleaning limit is 2 with status confirm", async () => {
    ...
    await swipe("swipeBtn", "right");
    await expectElementVisible(
      "Để đảm bảo sức khỏe cho CTV và chất lượng dịch vụ cho Khách hàng, bTaskee giới hạn mỗi CTV chỉ được nhận tối đa 2 công việc Vệ Sinh Văn Phòng/ngày.",
      "text"
    );
  });
});
