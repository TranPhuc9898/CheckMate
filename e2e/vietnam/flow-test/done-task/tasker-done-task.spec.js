...
describe('FILE: vietnam/flow-test/done-task/tasker-done-task.spec.js - Tasker done task thai land', () => {
  beforeEach(async () => {
    await device.launchApp();
    await initData('update-user/financialAccount', { phone: "0834567891", isoCode: "VN", financialAccountData: { FMainAccount: 100000, Promotion: 30000 } })
  ...
  it('LINE 67 - Tasker done task bpay', async () => {
    const ref = await initData('vn/task/createTask',
      {
        resetCollection: true,
        serviceName: 'CLEANING',
  ...
  it('LINE 122 - Tasker done task isPrepayTask', async () => {
    const ref = await initData('vn/task/createTask',
      {
        resetCollection: true,
        serviceName: 'CLEANING',
