...
describe("FILE: vietnam/flow-test/cancel-task/tasker-cancel-task-cleaning.spec.js - Tasker cancel task cleaning th", () => {
  beforeEach(async () => {
    ...
    await initData("update-user/financialAccount", {
      phone: "0834567891",
      isoCode: "VN",
      financialAccountData: { FMainAccount: 10000, Promotion: 10000 },
    });
  ...
  it("LINE 224 - Tasker cancel task CLEANING and status task change to CANCELED", async () => {
    await initData("vn/task/createTask", {
      resetCollection: true,
      serviceName: "CLEANING",
      viewedTaskers: ["0834567891"],
  ...
  it("LINE 275 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await tapId("reasonCancelWRONG_DATE");
    await expectElementVisible("cancelFee");
    await expectElementVisible("50,000₫", "text");
    await tapId("btnCancelTask");
    await expectElementVisible("cancelSuccess");
  ...
  it("LINE 345 - Tasker cancel task and status task change to CANCELED", async () => {
    ...
    await expectIdToHaveText('detailTransaction-0', '-100,000₫');
  });
  ...
  it("LINE 478 - Tasker cancel task and status task change to CANCELED", async () => {
    const ref = await initData("vn/task/createTask", {
  ...
  it("LINE 536 - Tasker cancel task and status task change to CANCELED", async () => {
    const ref = await initData("vn/task/createTask", {
      resetCollection: true,
      serviceName: "CLEANING",
      viewedTaskers: ["0834567891"],
    ...
    await expectElementVisible("Tài khoản chính", "text");
    // Được cộng thêm 800
    await expectElementVisible("80,000₫", "text");
    // Trừ 200 của 20% task
    await expectElementVisible("20,000₫", "text");
  });
});
